...
type propType = {
	id: number;
	post_id: string;
	prevPost: { content: string };
};
export default function PostOptionDropdown({ id, post_id, prevPost }: propType) {
	const [openEdit, setOpenEdit] = useState<boolean>(false);
	const { isLogedIn, csrfToken } = useContext(ContextAuth);
	const { get_comments } = useContext(ContextPost);

	const handleDelete = async () => {
		if (isLogedIn) {
			try {
				const response = await axiosInstance(`/api/blog/${id}/comment/detail`, {
					headers: {
			...
			{({ open }) => (
				<>
					...
					<Transition
						...
						<Menu.Items
							static
							className='origin-top-right absolute  mt-2 w-56 rounded-md shadow-lg bg-gray-800 ring-1 ring-black ring-opacity-5 focus:outline-none'>
							<div className='py-1'>
								<Menu.Item>
